...
from namingxref import NamingXref
...
class PolicyValidator:
    def __init__(self):
      ...
      self.naming_xref_flag = os.getenv("NAMING_XREF", "True")
    ...
    def validate(self, validation_payload: list, apiVersion: str, kind: str, test: str) -> str:
      ...
      """
      If self.naming_xref == "True" then generate names for resources resources.yaml and insert them in the policies in kyverno-test.yaml
      in the form of 'resources:' clauses.  
      
      The 'kyverno test' command cannot map resources to tests for policies unless each resource has a metadata.name clause. 
      That may not be the case because by the time PolicyValidator is invoked, Crossplane may not have generated the names for resources to be deployed.
      
      In this case, PolicyValidator can generate synthetic names and insert them in the corrresponding resource type tests to ensure 
      the 'kyverno test' command works properly/
      
      The NAMING_XREF environment variable stored in self.naming_xref_flag defaults to True. 
      """
      if self.naming_xref_flag == "True":
        analyze_resources_result = self.naming_xref.analyze_resources(self.target_folder)
        if analyze_resources_result:
           self.naming_xref.analyze_tests(self.target_folder)
